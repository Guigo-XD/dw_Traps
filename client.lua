...
AddEventHandler('dw_traps:placetrap', function(hash)
    ...
    if isPlacing == false then
        ...
        if hash == Config.FishTrapHash then
            RequestAnimDict("amb_wander@code_human_box_wander@male_a@base")
            while (not HasAnimDictLoaded("amb_wander@code_human_box_wander@male_a@base")) do
                Citizen.Wait(100)
            end
            TaskPlayAnim(myPed, "amb_wander@code_human_box_wander@male_a@base", "base", 8.0, -8.0, -1, 31, 0, true, 0,
                false, 0, false)
            AttachEntityToEntity(tempObj, myPed, 131, 0.05, 0.45, 0.08, 90.0, 90.0, 80.0, false, false, true, false, 0,
                true)
        else
            ...
            TaskPlayAnim(myPed, "amb_wander@code_human_hay_bale_wander@male_a@base", "base", 8.0, -8.0, -1, 31, 0, true,
                0, false, 0, false)
            AttachEntityToEntity(tempObj, myPed, 131, 0.05, 0.40, 0.09, 90.0, 70.0, 80.0, false, false, true, false, 0,
                true)
        end
        ...
        while placing do
            ...
            if PromptHasHoldModeCompleted(TrapPrompt) then
                print('hash', hash)
                if hash == Config.SmalltrapHash or (IsEntityInWater(myPed) and IsEntityInWater(tempObj)) then
                    PromptSetEnabled(TrapPrompt, false)
                    PromptSetVisible(TrapPrompt, false)
                    prompt = false
                    ...
                    myTraps[trapsCount] = { ["object"] = object, ['x'] = pPos.x, ['y'] = pPos.y, ['z'] = pPos.z,
                        ['stage'] = 0, ['hash'] = hash, ['tempgame'] = nil }
...
function placetrapcomplete(tempObj)
...
AddEventHandler('dw_traps:armtrap', function(itemn, hash)
    if isPlacing == false then
        ...
        if DoesEntityExist(object) then
            ...
            myArmedTraps[trapsCount] = { ["object"] = object, ['x'] = x, ['y'] = y, ['z'] = z, ['stage'] = 1,
                ['timer'] = Config.TimeToHarvest, ['hash'] = hash1, ['prompt'] = false, ['tempgame'] = nil }
...
AddEventHandler('dw_traps:fin2', function(object2, x, y, z, key, hash, itemname)
    print('Sua armadilha pegou algo!')
    TriggerEvent("redemrp_notification:start", 'Sua armadilha pegou algo!', 5)
    local trap = hash

    if hash == Config.SmalltrapHash then
        trap = Config.SmalltraFinishpHash
    end
    ...
    myCaughtTraps[trapsCount] = { ["object"] = object3, ['x'] = x, ['y'] = y, ['z'] = z, ['stage'] = 2,
        ['prompt'] = false, ['hash'] = hash, ['name'] = itemname, ['tempgame'] = nil }

    local game
    if trap == Config.FishTrapHash then
        game = 'mp007_p_catfish_sml01x'
        RequestModel(game)

        while not HasModelLoaded(game) do
            Citizen.Wait(1)
        end

        myCaughtTraps[trapsCount].tempgame = CreateObject(game, x, y, z, true, true, false)
        AttachEntityToEntity(myCaughtTraps[trapsCount].tempgame, myCaughtTraps[trapsCount].object, 0, 0.03, 0.0, 0.07,
            0.0, 00.0, 0.0, false, false, true, false, 0, true)
    else
        game = 'p_cs_rabbit01x'
        RequestModel(game)
        ...
        myCaughtTraps[trapsCount].tempgame = CreateObject(game, x, y, z, true, true, false)
        AttachEntityToEntity(myCaughtTraps[trapsCount].tempgame, myCaughtTraps[trapsCount].object, 0, 0.24, 0.0, 0.14,
            0.0, 80.0, 0.0, false, false, true, false, 0, true)
    end
    ...
    SetModelAsNoLongerNeeded(trap)
    SetModelAsNoLongerNeeded('mp007_p_catfish_sml01x')
end)
...
function Animation(time, text)
    ...
    TaskPlayAnim(playerpedid, "amb_work@world_human_farmer_weeding@male_a@idle_a", "idle_a", 8.0, -8.0, time, 1, 0, true,
        0, false, 0, false)
...
function HarvestTrap(key, itemname)
    if isPlacing == false then
        ...
        DeleteObject(myCaughtTraps[key].tempgame)
